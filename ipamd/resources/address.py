...
class AddressList(IPAMResource):
    ...
    def get(self, network_id: int, range_id: int):  # pylint: disable=R0201, W0613
        addresses = models.Address.query.filter_by(range_id=range_id).all()
        return addresses

    @marshal_with(models.Address.marshal_fields)
    def post(self, network_id: int, range_id: int):  # pylint: disable=R0201
...
class Address(IPAMResource):
    @marshal_with(models.Address.marshal_fields)
    def get(self, network_id: int, range_id: int, address_id: int):  # pylint: disable=R0201,W0613
        address = models.Address.query.filter_by(id=address_id, range_id=range_id).first_or_404()
        return address

    def delete(self, network_id: int, range_id: int, address_id: int):  # pylint: disable=R0201,W0613
        address = models.Address.query.filter_by(id=address_id, range_id=range_id).first_or_404()
