...
class AddressList(IPAMResource):
    ...
    def post(self, network_id, range_id):  # pylint: disable=R0201
        range_ = models.Range.query.filter_by(id=range_id, network_id=network_id).with_for_update().first()
        if not range_:
            abort(404, error='range not found')
...
class Address(IPAMResource):
    ...
    def get(self, network_id, range_id, address_id):  # pylint: disable=R0201,W0613
        address = models.Address.query.filter_by(id=address_id, range_id=range_id).first()
        if not address:
            abort(404)
        return address
